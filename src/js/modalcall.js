document.addEventListener('DOMContentLoaded', function () {
  const menuPhoneBtn = document.querySelector('.button__footer-phone')
  const navCallBtn = document.querySelector('.navigation__button-call') // Новая кнопка
  const callModal = document.querySelector('.modal-call__conteiner')
  const closeCallBtn = callModal.querySelector('.button__close')
  ...
  document.addEventListener('click', function (event) {
    if (
      ...
      !event.target.closest('.modal-call__conteiner') &&
      !event.target.closest('.button__footer-phone') &&
      !(isNavCallVisible() && event.target.closest('.navigation__button-call'))
    ) {
      closeModal()
    }
  })
